# This is the Supplementary Material of paper
# "The contribution of species–genetic diversity correlations to 
# the understanding of community assembly rules"
# Appendix S2
#
# R function to compute a decomposition of species-genetic diversity correlations 
# (SGDCs) into the contribution of different variables

SGDC.decomp <- function(SD, GD, FACTOR, plot=TRUE, verbose=TRUE)
  #
  # Description --
  # 
  # This method decomposes the covariance between 
  # species diversity (SD) and genetic diversity (GD) to identify 
  # the contributions of different habitat characteristics to the SGDC.
  # These contributions include direct contributions of site characteristics 
  # as well as the contributions of covariance between pairs of habitat characteristics.
  # 
  # if there is only one environmental variable you can add it as a numeric vector.
  # In such a case, the method compute the contribution of the single variables as 
# the product of the standardized regression coefficients of that variable on both SD and GD. 
# Note that in single linear regression, standardized regression coefficients equal 
# correlation coefficients between the response and the explanatory variable.
  ...
  if(dim(FACTOR)[2]==1){
    ...
    if(plot){
      ...
      barplot(Contribution$Percent, horiz = TRUE, col=c("red", "grey", "black"),
              main = "SGDC decomposition", xlab = "% of the SGDC", 
              xlim = c(min(Contribution$Percent)-10,max(Contribution$Percent)+10), 
              cex.names = rownames(Contribution))
    }
  }
